...
BBox1::BBox1(const Vector3 & min, const Vector3 & max) : m_min(min), m_max(max) {
	m_vbo_id = 0;
	m_idxvbo_id = 0;
	m_vbo_uptodate = 0;
}

void BBox1::clone(const BBox1 * source) {
	m_min = source->m_min;
	m_max = source->m_max;
	m_vbo_uptodate = 0;
}
...
void BBox1::add(const Vector3 & P) {
...
void BBox1::include(const BBox1 *bbb ) {
	add(bbb->m_min);
	add(bbb->m_max);
	m_vbo_uptodate = 0;
}
...
void BBox1::transform(const Trfm3D * T) {
