...
// Bounding Box
void imprimeBBox(const char *texto, const BBox1 *bb) {
	printf("      %s:", texto); bb->print(); printf("\n");
}

BBox1 * getBBox(const char *texto) {
	...
	bb = new BBox1(Vector3(xmin, ymin, zmin), Vector3(xmax, ymax, zmax));
...
//***
//***************************************************
void caseAABBs(const BBox1 *bb1, const BBox1 *bb2) {
	int result;

	result = BBoxBBoxIntersect(bb1, bb2);

	printf("\n Result for BBox-BBox:\n");
	imprimeBBox("BBox", bb1);
...
void caseAABBs1(const Vector3 & min1, const Vector3 & max1, const Vector3 & min2, const Vector3 & max2) {
	// First AABB
	BBox1 *bb1 = new BBox1(min1, max1);

	// Second AABB
	BBox1 *bb2 = new BBox1(min2, max2);
...
void caseAABBs2() {
	// First AABB
	BBox1 *bb1 = getBBox("BBox");

	// Second AABB
	BBox1 *bb2 = getBBox("BBox2");
...
void menuAABBs() {
	...
	while (fin == 0){
		...
		printf(" 4. AABB{(-1,-1,-1),(1,1,1)} and AABB{(1,1,1),(2,2,2)}\n");
...
//***
//***************************************************
void caseAABBplane(const BBox1 *bb, Plane *p) {
	int result;
...
void menuSpheres() {
	...
	while (fin == 0){
		...
		printf(" 2. Sphere1{(0,0,0), 1.0} and Sphere2{(1,1,1), 1.0}\n");
...
//***
//***************************************************
void caseSphereAABB(const BSphere *bs, const BBox1 *bb) {
	int result;
