...
void Trfm3D::add( const Trfm3D &T ) {
	...
	newR1X += this->m_c1[0] * T.m_c1[0];
	newR1X += this->m_c2[0] * T.m_c1[1];
	newR1X += this->m_c3[0] * T.m_c1[2];
	newR1X += this->m_tr[0] * T.m_d[0] ;

	newR1Y += this->m_c1[1] * T.m_c1[0];
	newR1Y += this->m_c2[1] * T.m_c1[1];
	newR1Y += this->m_c3[1] * T.m_c1[2];
	newR1Y += this->m_tr[1] * T.m_d[0] ;

	newR1Z += this->m_c1[2] * T.m_c1[0];
	newR1Z += this->m_c2[2] * T.m_c1[1];
	newR1Z += this->m_c3[2] * T.m_c1[2];
	newR1Z += this->m_tr[2] * T.m_d[0] ;

	newR2X = newR2Y = newR2Z = 0.0;

	newR2X += this->m_c1[0] * T.m_c2[0];
	newR2X += this->m_c2[0] * T.m_c2[1];
	newR2X += this->m_c3[0] * T.m_c2[2];
	newR2X += this->m_tr[0] * T.m_d[1] ;

	newR2Y += this->m_c1[1] * T.m_c2[0];
	newR2Y += this->m_c2[1] * T.m_c2[1];
	newR2Y += this->m_c3[1] * T.m_c2[2];
	newR2Y += this->m_tr[1] * T.m_d[1] ;

	newR2Z += this->m_c1[2] * T.m_c2[0];
	newR2Z += this->m_c2[2] * T.m_c2[1];
	newR2Z += this->m_c3[2] * T.m_c2[2];
	newR2Z += this->m_tr[2] * T.m_d[1] ;

	newR3X = newR3Y = newR3Z = 0.0;

	newR3X += this->m_c1[0] * T.m_c3[0];
	newR3X += this->m_c2[0] * T.m_c3[1];
	newR3X += this->m_c3[0] * T.m_c3[2];
	newR3X += this->m_tr[0] * T.m_d[2] ;

	newR3Y += this->m_c1[1] * T.m_c3[0];
	newR3Y += this->m_c2[1] * T.m_c3[1];
	newR3Y += this->m_c3[1] * T.m_c3[2];
	newR3Y += this->m_tr[1] * T.m_d[2] ;

	newR3Z += this->m_c1[2] * T.m_c3[0];
	newR3Z += this->m_c2[2] * T.m_c3[1];
	newR3Z += this->m_c3[2] * T.m_c3[2];
	newR3Z += this->m_tr[2] * T.m_d[2] ;
	...
	newTrX = 0;
	newTrX += this->m_scl * this->m_c1[0] * T.m_tr[0];
	newTrX += this->m_scl * this->m_c2[0] * T.m_tr[1];
	newTrX += this->m_scl * this->m_c3[0] * T.m_tr[2];
	newTrX += this->m_tr[0] * T.m_w;

	newTrY = 0;
	newTrY += this->m_scl * this->m_c1[1] * T.m_tr[0];
	newTrY += this->m_scl * this->m_c2[1] * T.m_tr[1];
	newTrY += this->m_scl * this->m_c3[1] * T.m_tr[2];
	newTrY += this->m_tr[1] * T.m_w;

	newTrZ = 0;
	newTrZ += this->m_scl * this->m_c1[2] * T.m_tr[0];
	newTrZ += this->m_scl * this->m_c2[2] * T.m_tr[1];
	newTrZ += this->m_scl * this->m_c3[2] * T.m_tr[2];
	newTrZ += this->m_tr[2] * T.m_w;
...
Vector3 Trfm3D::transformPoint(const Vector3 & P) const {
    /*
	Vector3 res;
    res.x() = P.x()*m_c1.x()*m_scl + P.y()*m_c2.x()*m_scl + P.z()*m_c3.x()*m_scl + m_tr.x();
	res.y() = P.x()*m_c1.y()*m_scl + P.y()*m_c2.y()*m_scl + P.z()*m_c3.y()*m_scl + m_tr.y();
	res.z() = P.x()*m_c1.z()*m_scl + P.y()*m_c2.z()*m_scl + P.z()*m_c3.z()*m_scl + m_tr.z();
	return res;
    */

    Vector3 res;
    //Px' = Px*m_c1[0]*m_scl + Py*m_c2[0]*m_scl + Pz*m_c3[0]*m_scl + m_tr[0]
    res[0] = P[0] * m_c1[0] * m_scl + P[1] * m_c2[0] * m_scl + P[2] * m_c3[0] * m_scl + m_tr[0];
    //Py' = Px*m_c1[1]*m_scl + Py*m_c2[1]*m_scl + Pz*m_c3[1]*m_scl + m_tr[1]
    res[1] = P[0] * m_c1[1] * m_scl + P[1] * m_c2[1] * m_scl + P[2] * m_c3[1] * m_scl + m_tr[1];
    //Pz' = Px*m_c1[2]*m_scl + Py*m_c2[2]*m_scl + Pz*m_c3[2]*m_scl + m_tr[2]
    res[2] = P[0] * m_c1[2] * m_scl + P[1] * m_c2[2] * m_scl + P[2] * m_c3[2] * m_scl + m_tr[2];
    return res;

}

// Transformar un vector, quitando parte final de translacion.

Vector3 Trfm3D::transformVector(const Vector3 & V) const {
	/*
    Vector3 res;
    res.x() = V.x()*m_c1.x()*m_scl + V.y()*m_c2.x()*m_scl + V.z()*m_c3.x()*m_scl;
	res.y() = V.x()*m_c1.y()*m_scl + V.y()*m_c2.y()*m_scl + V.z()*m_c3.y()*m_scl;
	res.z() = V.x()*m_c1.z()*m_scl + V.y()*m_c2.z()*m_scl + V.z()*m_c3.z()*m_scl;
	return res;
    */
    Vector3 res;
    //Vx' = Px*m_c1[0]*m_scl + Py*m_c2[0]*m_scl + Pz*m_c3[0]*m_scl
    res[0] = V[0] * m_c1[0] * m_scl + V[1] * m_c2[0] * m_scl + V[2] * m_c3[0] * m_scl;
    //Vy' = Px*m_c1[1]*m_scl + Py*m_c2[1]*m_scl + Pz*m_c3[1]*m_scl
    res[1] = V[0] * m_c1[1] * m_scl + V[1] * m_c2[1] * m_scl + V[2] * m_c3[1] * m_scl;
    //Vz' = Px*m_c1[2]*m_scl + Py*m_c2[2]*m_scl + Pz*m_c3[2]*m_scl
...
void Trfm3D::setRotZ(float angle ) {
	...
	m_c2 = Vector3( -sinf(angle), cosf(angle),  0.0f );
...
/*
  Creamos matriz que rota por un angulo (theta) alrededor de un punto (VV)
...
void Trfm3D::setRotVec(const Vector3 & VV, float theta ) {
	...
	//valores centrales, vamos, la rotacion
    m_c1.x() = (cosTheta + (1 - cosTheta) * aux.x() * aux.x());
    m_c2.x() = ((1 - cosTheta) * aux.x() * aux.y() - aux.z() * sinTheta);
    m_c3.x() = ((1 - cosTheta) * aux.x() * aux.z() + aux.y() * sinTheta);

    m_c1.y() = ((1 - cosTheta) * aux.x() * aux.y() + aux.z() * sinTheta);
    m_c2.y() = (cosTheta + (1 - cosTheta) * aux.y() * aux.y());
    m_c3.y() = ((1 - cosTheta) * aux.y() * aux.z() - aux.x() * sinTheta);

    m_c1.z() = ((1 - cosTheta) * aux.x() * aux.z() - aux.y() * sinTheta);
    m_c2.z() = ((1 - cosTheta) * aux.y() * aux.z() + aux.x() * sinTheta);
    m_c3.z() = (cosTheta + (1 - cosTheta) * aux.z() * aux.z());

	//rellenamos resto de la matriz 4x4
	m_scl = 1.0f;
	m_tr = Vector3::ZERO;
	m_d = Vector3::ZERO;
	m_w = 1.0f;
...
// Rotar un angulo en radianes sobre un eje definido por un vector y un pto en el espacio
// Movemos del pto al origen - Es en positivo por como funciona el add() que multiplica de derecha a izq
// Asi se pone la P a la derecha, luego la mult y luego la -P, quedando asi: -P * Rot * P; MovCentro, Rot, MovToPos
// Luego se hace la rotacion en el (0,0) y al final lo devolvemos a su pto en el espacio
...
void Trfm3D::setRotAxis(const Vector3 & V, const Vector3 & P, float angle ) {
	...
	Vector3 aux = V;
	aux.normalize();
	// Ponemos positivo y luego negativo para multiplicar por la derecha
	setTrans(P);
	addRotVec(aux, angle);
	addTrans({-P.x(),-P.y(),-P.z()});//volvemos a dejar en el pto que estuviera
	*/
    setTrans(P);
    addRotVec(V, angle);
    addTrans(-1.0f * P);
...
// Dada una transformacion T(definida por un pto y un vector) realizamos una rotacion con la nueva
// transformacion localT que deja el resultado en T.
//         T = T * localT
